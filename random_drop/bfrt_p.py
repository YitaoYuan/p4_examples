def Namespace():
    ...
    for worker in range(len(port)):
        bfrt.random_drop.pipe.Ingress.l2_forward_table.add_with_l2_forward(MAC[worker], port[worker])

    bfrt.random_drop.pipe.Egress.dcqcn.wred.add(0, 0, 125, 2500, 0.01)
    # DCQCN
    # 0 ~ 10KB, 0 
    # 10 ~ 200KB, 0 ~ 0.01
    # 200KB ~, 1
    ...
    bfrt.random_drop.pipe.Ingress.l2_forward_table.add_with_l2_multicast(0xffffffffffff, mgid)
